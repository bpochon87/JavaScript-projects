...
const Calculator =
{
    ...
    First_Operand: null,
    // This checks whether or not the second operand has been entered.
...
function Input_Digit(digit)
{
    ...
    {
        // This overwrites Display_Value if current value is 0. Else it adds onto it.
...
{
    // This ensures that accidental clicking of the decimal point won't cause bugs.
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot))
    {
        // If Display_Value doesn't contain decimal point, we want to add one.
...
function Handle_Operator(Next_Operator)
{
    ...
    // When operator key pressed, convert current number displayed on screen to a number
    // and store result in Calculator.First_Operand if not existing already.
    const Value_of_Input = parseFloat(Display_Value);
    // Checks if operator already exists and if Wait_Second_Operand is true then updates
    // operator and exits function
...
keys.addEventListener('click', (event) =>
{
    ...
    const { target } = event;
    // If element that was clicked isn't a button, exit function.
